...
import { SerieGridSkeleton } from "@/app/[locale]/(public)/series/_components/serie-card-skeleton";
...
import { SortBy } from "@/app/[locale]/(public)/series/_components/sort-by";
...
export default function Page() {
  ...
  const { data: seriesData, isLoading, isFetching } = useSeries({
  ...
  const smartSuggestions = series.slice(0, 3);
          ...
          <div className="glass-panel rounded-2xl p-4">
            ...
            <div className="mt-3 space-y-2 text-xs text-muted-foreground">
              {debouncedSearch && smartSuggestions.length > 0 ? (
                smartSuggestions.map((serie) => (
                  <div
                    key={serie.id}
                    className="flex items-center justify-between rounded-lg border border-white/10 bg-white/5 px-3 py-2"
                  >
                    <span className="line-clamp-1">{serie.name}</span>
          ...
          {series.length > 0 && (
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-6">
              {series.map((serie, index) => (
                <StaggerGridItem key={serie.id} index={index}>
