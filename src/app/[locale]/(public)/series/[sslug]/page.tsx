...
import { useFirstEpisode } from "@/app/[locale]/(public)/series/_hooks/use-first-episode";
...
export default function SerieDetailPage({ params }: SerieDetailPageProps) {
  ...
  const {
    ...
    error: episodeError,
  } = useFirstEpisode(serieIdValue);

  useEffect(() => {
    if (!isSerieLoading && !isEpisodeLoading && serie && firstEpisode) {
      const lastWatched = getLastWatchedEpisode(serieIdValue);
