"use client";

import { Card } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import Image from "next/image";
import { useRouter } from "next/navigation";
import { Serie } from "@/models";
import { generateSlug } from "@/lib/slug";

interface SerieCardProps {
  serie: Serie;
}

export function SerieCard({ serie }: SerieCardProps) {
  const router = useRouter();
  const serieSlug = generateSlug(serie.id, serie.name);

  const handleClick = () => {
    router.push(`/series/${serieSlug}`);
  };

  return (
    <Card
      className="group relative overflow-hidden p-0 rounded-md gap-2 cursor-pointer transition-all hover:shadow-lg"
      onClick={handleClick}
    >
      <div className="aspect-[4/5] relative overflow-hidden rounded-t-md">
        <Image
          src={serie.posterPath || "/placeholder.png"}
          alt={serie.name}
          fill
          className="object-cover transition-transform group-hover:scale-110 scale-105"
        />
        <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
        <div className="absolute bottom-1 right-1">
          <Badge className="text-xs rounded-sm" variant="secondary">
            {serie.lastAirDate ? "Completed" : "Ongoing"}
          </Badge>
        </div>
      </div>
      <div className="p-2 pt-0">
        <h3 className="text-sm font-medium line-clamp-1">{serie.name}</h3>
        <span className="text-xs text-muted-foreground">
          {serie.firstAirDate
            ? new Date(serie.firstAirDate).toLocaleDateString("en-US", {
                month: "short",
                day: "2-digit",
                year: "numeric",
              })
            : "TBA"}
        </span>
      </div>
    </Card>
  );
}
